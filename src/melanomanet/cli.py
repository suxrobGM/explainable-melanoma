...
app = typer.Typer(help="MelanomaNet: explainable melanoma detection.", no_args_is_help=True)
...
def run_seeds(
    config: ConfigOpt = "config.yaml",
    seeds: Annotated[str, typer.Option(help="Comma-separated seeds, e.g. 25,26,27")] = "25,26,27",
...
def faithfulness(
    ...
    steps: Annotated[int, typer.Option(help="Points along deletion/insertion curves")] = 20,
    iou_quantile: Annotated[
        float, typer.Option(help="Attention quantile treated as 'high' for IoU")
    ] = 0.8,
    seed: Annotated[int, typer.Option(help="Seed for the random-attention control")] = 25,
) -> None:
    ...
    evaluate_faithfulness(load_config(config), checkpoint, input_dir, steps, iou_quantile, seed)
...
def infer(
    ...
    input_dir: Annotated[str | None, typer.Option(help="Directory containing images")] = None,
) -> None:
...
def generate_concepts(
    ...
    max_samples: Annotated[int, typer.Option(help="Maximum samples per concept class")] = 200,
    min_samples: Annotated[int, typer.Option(help="Minimum samples required per class")] = 50,
) -> None:
...
def paper_figures(
    ...
    cfg = load_config(config)
    generate_paper_figures(Path(output_dir or cfg.paths.output_dir), Path(paper_output))
