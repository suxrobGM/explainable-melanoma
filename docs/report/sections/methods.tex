...
forward passes to estimate uncertainty. The dropout layers are kept active at
inference while batch-normalisation layers remain in evaluation mode, so that
each pass draws a sample from an approximate posterior over the network weights
without making single-image predictions depend on batch statistics. From the $T$
sampled distributions we compute three uncertainty measures that satisfy the
decomposition predictive $=$ aleatoric $+$ epistemic:
...
\textbf{Aleatoric Uncertainty} captures inherent data noise and is the mean
entropy of the individual passes:
\begin{equation}
    \mathbb{E}[H] = \frac{1}{T}\sum_{t=1}^T H[p_t].
\end{equation}
It reflects uncertainty due to ambiguous or overlapping features in the input
itself and persists regardless of the amount of training data.

\textbf{Epistemic Uncertainty} measures model uncertainty arising from limited
training data. We quantify it as the mutual information between the prediction
and the model parameters, which for this decomposition equals the difference
between the predictive entropy and the aleatoric term:
\begin{equation}
    I = H[\bar{p}] - \frac{1}{T}\sum_{t=1}^T H[p_t].
\end{equation}
It is large when the sampled passes disagree---that is, when the
dropout-induced posterior is not concentrated---and can in principle be reduced
by adding training examples similar to the input.
